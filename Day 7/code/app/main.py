...
#call query_items: http://localhost:8000/query_items?item_id=1
@app.get("/query_items")
def read_item(item_id: int):
   return {"item_id": item_id}
...
#regex tells us it needs to have @
@app.get("/text_model/")
...
@app.post("/cv_model/")
async def cv_model(data: UploadFile = File(...), h: Optional[int] = 256, w: Optional[int] = 256):
